...
class KeywordExtractorByList(WaterfallKeywordExtractor):
    def build(self):
        ...
        self.doc1 = nlp("slow, super slow, snail, unhurried, leisurely, measured, moderate, deliberate, steady, sedate, slow-moving, slow-going, easy, relaxed, unrushed, gentle, undemanding, comfortable, ponderous, plodding, laboured, dawdling, loitering, lagging, laggard, sluggish, sluggardly, snail-like, tortoise-like, leaden-footed, leaden, creeping, laggy, lollygagging, calm, gently, docile, friendly, easy, dull, tediously, lazy, sleepily, tardy, indolent, graceful, largo, adagio, sluggish, relaxed, casual, belatedly, tardily, ritardando, latterly, lately, lenient, poor, dully, lethargically")
        self.doc2 = nlp("
...
class UnifiedKeywordExtractor(WaterfallKeywordExtractor):
    def build(self,target_words):
        self.word_to_words_matcher = WordToWordsMatcher().build()
        self.keyword_extractor_by_list = KeywordExtractorByLis().build()

    def predict(self,sentence):
        d = self.keyword_extractor_by_list.predict(sentence)

        matched_words = self.word_to_words_matcher.predict(sentence.split(' ')) #TODO: use a more sophisticated tokenizer
